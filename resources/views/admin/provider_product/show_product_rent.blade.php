...
@section('title', __('Product Details For Rent'))
                            ...
                            <p><strong>{{ __('Product Images') }}:</strong>
                                <div style="display: flex; gap: 10px; flex-wrap: wrap;">
                                    @foreach ($product->images as $img)
                                        <img src="{{ asset('storage/'.$img->image) }}" alt="{{ __('Product Image') }}" style="width: 150px; height: auto; border-radius: 5px;">
                                    @endforeach
                                </div>
                            </p>

                            <a href="{{ route('Admin.product_rent') }}" class="btn btn-secondary">{{ __('Back') }}</a>
                        </div>
                    </div>
                </div>
            </div>
